data {
  int<lower=0> N_subj;         //number of samples
  int<lower=0> N_otu;         //number of otus
  int<lower=0> N_family;
  int<lower=0> N_variables; //Number of variables + intercept
  int<lower=0> y[N_subj,N_otu];   //otu count
  matrix[N_subj, N_variables] x;   //value of x per sample
  vector[N_subj] offset;
  int index_family[N_otu];
}
parameters {
